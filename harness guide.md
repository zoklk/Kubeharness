...
| smoke_test_path | `edge-server/tests/<phase>/smoke-test-<sub_goal>.sh` (없으면 skip) |

→ Runtime Verifier 결정적 게이트 함수 시그니처:
```python
def run_runtime_phase1(service_name: str, sub_goal_name: str, phase_name: str) -> dict:
    namespace = "gikview"
    ...
    smoke_test = Path(f"edge-server/tests/{phase_name}/smoke-test-{sub_goal_name}.sh")
...
4. smoke test 실행 (`edge-server/tests/<service>/smoke-test-<sub_goal>.sh` 존재 시)
   - exit_code != 0 → fail
