...
- **개발**: `{NAMESPACE}` 고정. 모든 리소스가 여기 배포됨
- 다른 네임스페이스 사용 금지
...
- 형식: `<service>-dev-v1`
- `dev`는 개발 환경 식별자. 네임스페이스(`{NAMESPACE}`)와 구분되는 개념
...
env:
  ...
  - name: EMQX_NODE__NAME
    value: "emqx@$(POD_NAME).emqx-headless.{NAMESPACE}.svc.<domain_suffix>"
    #              ^^^^^^^^^^^ Kubernetes가 파드 기동 전에 치환
    # domain_suffix는 ## Cluster Environments 섹션 참조
```
