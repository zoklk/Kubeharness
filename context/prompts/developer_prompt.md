...
When a `## Previous Diagnostic Findings (Unverified) [<name>]` section appears, it contains **low-confidence** hints auto-generated by Runtime Verifier from past failures:

- Use as debugging hints, not authoritative specs
- If a suggestion contradicts Technology Knowledge, prefer Technology Knowledge
- Do not blindly apply all suggestions — reason about which ones apply to the current failure
