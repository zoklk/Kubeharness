...
> **디렉토리 루트**: harness 코드(`GikView/`)와 프로젝트 파일(`gikview/`)이 분리되어 있다.
> - **HARNESS_ROOT** = `/home/netai/GikView/` — harness 코드, 시스템 프롬프트, LLM 설정
> - **PROJECT_ROOT** = `/home/netai/gikview/` — Helm 차트, context 문서, 지식 파일, 로그
> - `HARNESS_PROJECT_DIR` 환경변수로 PROJECT_ROOT 오버라이드 가능 (다른 프로젝트 연결 시)
...
| Conventions | `gikview/context/base/conventions.md` | 파일 경로 규칙, 릴리스 이름 등 |
| Tech Stack | `gikview/context/base/tech_stack.md` | 버전, 컴포넌트 정보 |
| Cluster Environments | `gikview/config/cluster.yaml` → Python 코드 주입 | active 환경, dev/prod domain_suffix, arch |
| Sub-Goal Specification | `gikview/context/phases/<phase>.md` 에서 sub_goal 섹션 추출 | 목표 사양, 인터페이스, 제약사항 |
| Existing Files | `gikview/{PREFIX}{helm,docker,ebpf}/<service_name>/` 스캔 | 파일 경로 목록만 주입. 내용은 `read_file` 툴로 조회 |
| Smoke Tests | `gikview/{PREFIX}tests/<phase>/smoke-test-<sub_goal>.sh` | 파일 전체 내용 직접 주입. 없으면 생략. |
| Dependency Services | sub_goal spec의 `dependency` 필드 파싱 | 의존 서비스명 목록 + kagent로 직접 조회 안내 |
| Technology Knowledge | `gikview/context/knowledge/<technology_name>.md` + dep knowledge | 기술 기반 지식 (고신뢰). 없으면 생략. |
...
**LLM에게 없는 정보**: `gikview/config/cluster.yaml` 직접 읽기 불가. 단, Cluster Environments 섹션에 active env, domain_suffix, arch가 주입되므로 이를 활용해야 함.
...
루프 종료 후 `_parse_artifacts()`가 `None`을 반환하면(LLM이 자연어/마크다운을 응답한 경우) `request_json_response()`를 호출해 최대 **2회** JSON 재요청을 시도한다. 재요청이 모두 실패하면 `notes`에 파싱 실패 메시지를 기록하고 파일 쓰기는 건너뛴다.

### 사용 가능 툴

- **kagent MCP** (`developer_tools`): `GetResources`, `GetResourceYAML`, `DescribeResource`, `GetPodLogs`, `GetRelease` 등 (read-only)
- **`read_file`** (로컬): `gikview/{PREFIX}` 하위 파일 읽기. MCP 불필요. Existing Files 목록의 파일 내용 조회용.

### 허용 경로 (write guard)

파일 쓰기 시 두 단계 검증:
1. `{PREFIX}` 미준수 → 드롭 (경고 출력)
2. 허용 서브디렉터리(`helm/`, `docker/`, `ebpf/`) 외 경로 → 드롭
...
3. docker 감지 시: hadolint → gitleaks
4. 아무것도 감지 안 되면 `artifact_detection` fail
...
| 단계 | 조건 | 동작 |
|------|------|------|
| docker build+push | `gikview/{PREFIX}docker/<service>/Dockerfile` 존재 | `gikview/config/build.yaml`의 `registry`로 빌드 후 푸시 |
| helm upgrade --install | `gikview/{PREFIX}helm/<service>/` 존재 | `--wait` 없음 (빠른 적용) |
| kubectl wait pods | chart에 workload 리소스(Deployment/StatefulSet/DaemonSet 등) 있을 때만 | **2단계**: 60s 대기 → terminal 상태 감지 → terminal이면 즉시 fail / 아니면 240s 추가 대기. CRD-only chart는 `helm template` 분석 후 자동 skip |
| smoke test | `gikview/{PREFIX}tests/<phase>/smoke-test-<sub_goal>.sh` 존재 시 | kubectl_wait 실패 시 skip |

**values 파일**: `values.yaml` + `values-{active}.yaml` (static_verifier와 동일 로직)

**배포 방식**: 항상 `helm uninstall` 후 `helm upgrade --install`. release not found(exit_code=1)는 정상으로 처리하며 `helm_uninstall` 체크를 추가하지 않음. uninstall 성공 시에만 체크 추가.
...
- user message 주입 컨텍스트 (순서대로):
  ...
  | Artifact Files | `{PREFIX}{helm,docker}/<service>/` 스캔 | 정확한 파일 경로 참조 |
  | Technology Knowledge | `gikview/context/knowledge/<tech>.md` + dep knowledge (`harness.llm.context.read_knowledge`) | 기술 기반 지식, 환경별 설정값 (없으면 생략) |
- Phase 2가 `files` 필드를 채우면 하네스가 직접 파일 쓰기 후 자가 루프 재배포
- **파일 쓰기 제한**: `{PREFIX}helm|docker|ebpf/` 만 허용. `{PREFIX}tests/` 는 하네스가 차단 (`_ALLOWED_WRITE_SUBDIRS`).
...
- 루프 종료 후 `extract_json_dict()`가 `None`을 반환하면(자연어/마크다운 응답) `request_json_response()`를 호출해 최대 **2회** JSON 재요청을 시도한 뒤 `_parse_phase2()`에 위임한다.
...
developer 노드가 `gikview/context/phases/<phase>.md`에서 sub_goal 섹션을 추출하는 방법:
...
- 못 찾으면 파일 전체를 반환.
- `**service_name**:` 필드 → helm/docker/smoke 경로의 기준
- `**technology**:` 필드 → knowledge 파일 조회 키 (`gikview/context/knowledge/<technology>.md`). 없으면 service_name 폴백.
...
- [ ] `service_name`이 helm/docker 경로의 `<service_name>`과 일치하는가
- [ ] `**technology**:` 필드가 명시되어 있는가 (service_name과 다를 경우 필수. 예: service=cilium-l2, tech=cilium)
- [ ] 기술 일반 지식(동작 원리, 설정 예시, 버전 근거)은 phase 문서가 아닌 `gikview/context/knowledge/<tech>.md`에 작성했는가
...
### 프로파일 (`GikView/config/llm.yaml`)

모든 LLM 설정은 `GikView/config/llm.yaml`(HARNESS_ROOT)의 프로파일로 중앙 관리된다. 프로젝트와 무관한 harness operator 설정이므로 `gikview/`가 아닌 `GikView/`에 위치한다.

```yaml
node_profiles:
  developer: default                    # developer 노드 → default 프로파일
  runtime_verifier_phase2: claude-verifier  # Phase 2 LLM → claude-verifier 프로파일
  # runtime_verifier_phase2: gemma-verifier  # Gemma로 교체 시
...
profiles:
  ...
  gemma-verifier:              # Gemma + Google Search grounding
    provider: gemini
    model: gemma-4-31b-it
    max_tool_turns: 5
    tool_timeout: 60
    tool_result_max_chars: 3000
    web_search:
      max_uses: 5              # tool_type은 Gemini에서 무시됨
...
### JSON 재요청 (`request_json_response`)

`harness/llm/tool_loop.py`에 중앙화된 헬퍼. developer·runtime_verifier 양쪽이 공유한다.

```
request_json_response(messages, profile, schema_hint, max_retries=2)
  → (dict | None, updated_messages)
```

- `run_tool_loop()` 직후 JSON 파싱 실패 시 호출
- 기존 대화 히스토리에 user 재요청 메시지를 append → `llm.chat()` 호출 → `extract_json_dict()` 파싱 재시도
- 성공 시 `(dict, msgs)` 반환; `max_retries` 소진 시 `(None, msgs)` 반환
- 진행 상황은 Rich `[dim]` 스타일로 콘솔 출력 (`⟳ JSON 재요청 N/M ...`, `✓/✗ JSON 재요청 결과`)

### Web Search

프로파일에 `web_search` 섹션이 있으면 Runtime Verifier Phase 2에서 항상 활성화된다. 검색 지침은 system prompt(`context/prompts/runtime_verifier_prompt.md`)에 포함된다 — user message 중복 주입 없음.

프로바이더별 구현:

| provider | 방식 | 비고 |
|----------|------|------|
| `anthropic` | Anthropic 내장 도구 (`web_search_20260209`) — `tool_type` 키로 API 버전 지정 | API에 built-in tool로 전달 |
| `gemini` | Google Search Grounding (`types.Tool(google_search=...)`) — `tool_type` 키 무시 | Gemini API 내부 처리, tool_loop 거치지 않음 |

`_chat_gemini()`는 tools 리스트에서 `"type"` 필드가 있는 항목을 Anthropic 내장 도구로 인식해 `name="web_search"`인 경우 Google Search grounding으로 자동 변환한다.
