...
이 문서는 Kubeharness 레포 리팩터링 지시문이다. 임의 판단으로 설계 변경 금지.
...
1. **하드코딩 최소화** — namespace, release naming, chart path, 워크스페이스 루트 디렉토리명, 체크 목록, 타임아웃 등 **모든 프로젝트별 값은 `config/harness.yaml` 한 곳에만** 정의. Python 코드, hook, slash command, skill 모두 이 설정을 읽는다. 코드·템플릿에 `"gikview"`, `"edge-server"`, `"{service}-dev-v1"` 같은 리터럴이 나타나면 안 된다. 템플릿 파일의 워크스페이스 경로는 `{{workspace_dir}}` (init 치환) 또는 `{workspace}` (런타임 치환) 플레이스홀더로 둔다.
2. **파이프라인 단계 기준 분류** — 파일을 tool 타입(kubectl/helm/shell)이 아닌 **언제 실행되는가**로 묶는다. 배포 전 정적 검사는 `static.py`, 배포 실행 + 배포 후 검증은 `runtime.py`. 유지보수자가 "배포 후 새 체크 추가" 같은 요구를 받았을 때 **열어볼 파일이 즉시 결정**되도록.
3. **도구 중립성** — Python·config·`AGENTS.md`·hook 스크립트는 어떤 에이전트 CLI 에서도 동작해야 한다. Claude Code 전용 기능에 의존 금지. `.claude/` 는 wiring 일 뿐 핵심 아님.
4. **최소주의** — 빈 파일·추상 레이어·선행 최적화 금지. `__init__.py` 제외 빈 스캐폴딩 만들지 않는다. 필요해지는 순간에 추가.
5. **LLM 분리** — Python 코드에서 LLM API 를 직접 호출하지 않는다. LLM 판단은 모두 에이전트 CLI 본체 또는 subagent 담당.
6. **결정적 CLI 의 출력 제어** — CLI 는 stdout 에 **JSON 요약 + 실패 항목의 log_tail** 만 뱉는다. 전체 로그는 **세션 로그 파일 하나**로 저장하고 경로만 알림. 에이전트 컨텍스트 오염 방지.
7. **Skills 는 참조 문서** — 기존 `developer_prompt.md`, `runtime_verifier_prompt.md` 는 `AGENTS.md` 에 녹이지 말고 **작업 단위별 SKILL.md** 로 쪼개서, 해당 작업을 하는 주체(메인 세션·subagent)가 필요할 때만 로드하도록 한다 (progressive disclosure).
8. **한 번의 `/deploy` = 한 개의 세션 로그** — 정적 검사, 배포, 동적 검증, 진단, 재시도까지 한 사이클의 모든 출력은 **단일 세션 로그 파일**에 순차 기록. 도구별 개별 로그는 만들지 않는다. 관측성 단순화.
...
소비자는 Kubeharness 코드를 수정하지 않는다. pip 로 업데이트 받고, 자기 프로젝트의 `.claude/`, `AGENTS.md`, `config/harness.yaml` 만 관리.
...
`harness.yaml.example.tmpl` 은 예시 값을 담되, **필수 재정의 대상**을 주석으로 표시한다. 아래 값은 모두 소비자 프로젝트에서 덮어써야 한다.
...
conventions:
  ...
  # 기본 "workspace". 프로젝트가 이미 쓰는 이름(예: "edge-server", "src", "apps")으로 바꿀 수 있다.
  # 아래 *_path 의 `{workspace}` 는 런타임에 이 값으로 치환된다.
...
`load_config()` 는 프로세스 내부에서 `@lru_cache(maxsize=1)` 로 결과 캐시. 한 CLI 호출 내에 YAML 을 여러 번 파싱하지 않는다. 테스트에서 config 파일을 교체해야 하면 `load_config.cache_clear()` 사용.
...
모든 외부 명령은 이 함수를 거친다. `subprocess.run` 다른 곳에서 직접 호출 금지.
...
- namespace·release·path 값은 `cfg.resolve(service)` 에서만 얻는다.
...
- 복사 후 안내 메시지:
  - "`config/harness.yaml` 을 프로젝트에 맞게 수정하세요 (특히 `cluster.namespace`, `conventions.registry`, `environments.*`)"
  - "`<workspace_dir>/` 디렉토리에 helm/docker/tests 하위를 구현하세요"
  - "`AGENTS.md` 의 프로젝트별 규칙 섹션을 보완하세요"
- 최소 의존성: stdlib 만
...
- `deploy-orchestrator` 는 세션 시작 시 `SESSION_ID` 를 계산해 **LLM 메모리에 보관**하고, 이후 모든 Bash 호출마다 **inline 환경변수 주입**:
  ...
  ```
  Claude Code `Bash` 도구는 호출마다 새 shell 을 스폰하므로 `export` 는 호출 간 지속되지 않는다. 매 호출마다 같은 경로 문자열을 prefix 해 주입해야 한다.
...
- 도구 허용:
  ...
  tools:
    ...
    # <tool> 은 kagent 서버 노출 이름(snake_case)이다 — 소비자 `config/kagent.yaml` 참고.
...
- 시스템 프롬프트 (사이클 규칙):
  ...
  **step-diagnose (runtime 실패 시)**
  1. `Task` 도구로 `runtime-diagnoser` 호출. Claude Code `Task` 는 prompt 문자열만 받으므로, 입력을 마크다운/JSON 블록으로 직렬화해 prompt 본문에 담는다:
     ```
...
- **static 실패는 orchestrator 가 재시도하지 않는다**. 초기 코드 문법 오류는 메인 세션 책임.
...
이 섹션은 refactor.md 만 읽고 구현할 수 있게 기존 코드에서 반드시 확인·이식할 항목을 명시한다. **본 설계를 바꾸지 않고 이식만** 한다.
...
4. **실패 시**: build/push 실패면 `apply()` 는 즉시 중단 — helm 단계로 넘어가지 않는다.
...
- **실패 시**: helm 자체 exit ≠ 0 이면 apply fail. verify_runtime 으로 넘어가지 않는다.
...
이식 시 위 로직은 **그대로 복사**, 단 namespace/release/path 리터럴만 `cfg.resolve(service)` 참조로 바꾼다.
