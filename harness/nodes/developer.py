...
def _build_user_message(
    ...
    for title, content in _read_knowledge(technology_name, deps):
...
def _read_knowledge(technology_name: str, dep_names: list[str]) -> list[tuple[str, str]]:
    """
    ...
    for name in [technology_name] + dep_names:
        baseline = knowledge_dir / f"{name}.md"
        if baseline.exists():
            content = baseline.read_text(encoding="utf-8").strip()
            if content:
                results.append((f"## Technology Knowledge: {name}", content))

        findings = knowledge_dir / f"{name}-llm-findings.md"
        if findings.exists():
