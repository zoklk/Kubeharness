...
_DEFAULT_SUBDIRS = ("helm", "docker", "ebpf")

# LLM이 쓸 수 있는 하위 디렉터리 허용 목록.
# edge-server/tests/ 는 smoke test 보호를 위해 의도적으로 제외.
_ALLOWED_WRITE_SUBDIRS = ("helm/", "docker/", "ebpf/")
...
def write_files(
    ...
    Phase 1 — Pre-validation:
        ...
        - 허용 서브디렉터리(helm/docker/ebpf) 외 경로 → drop (tests/ 등 차단)
        - 빈 content → drop
    ...
    for f in files:
        ...
        if not any(rel.startswith(sub) for sub in _ALLOWED_WRITE_SUBDIRS):
            _con.print(f"  [red]⚠ write blocked (allowed: helm/docker/ebpf) — dropped:[/red] {path!r}")
            continue
        if not content:
